defmodule AthenaWeb.ItemControllerTest do
  use Athena.DataCase
  ...
  describe "index" do
    ...
    test "lists all items", %{conn: conn, item_group: item_group} do
      conn = get(conn, Routes.item_path(conn, :index, item_group.id))
  ...
  describe "new item" do
    ...
    test "renders form", %{conn: conn, item_group: item_group} do
      conn = get(conn, Routes.item_path(conn, :new, item_group.id))
  ...
  describe "create item" do
    ...
    test "redirects to show when data is valid", %{conn: conn, item_group: item_group} do
      conn = post(conn, Routes.item_path(conn, :create, item_group.id), item: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, item_group: item_group} do
      conn = post(conn, Routes.item_path(conn, :create, item_group.id), item: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Item"
    end
  end
  ...
  describe "delete item" do
    ...
    test "deletes chosen item", %{conn: conn, item: item, item_group: item_group} do
      ...
      assert redirected_to(conn) == Routes.item_path(conn, :index, item_group.id)
